...
- 原生批量命令是服务端实现的，而 pipeline 需要服务端与客户端共同完成
...
- pipeline 缓冲的指令只是会依次执行，不保证原子性，如果执行过程中发生异常，会继续执行后续的指令
- 使用 pipeline 组装的命令个数不能太多，不然数据量过大客户端阻塞的时间可能过久，同时服务端也被迫回复一个队列答复，占用很多内存。